...
TEST_CASE("Channel invalid dimension") {
  ...
  CHECK(ret_decode.has_value());
  auto pixels = original.pixels();
  ChannelMgr channel_mgr{inputimg.width, -1, inputimg.components};
  auto ret_split = channel_mgr.splitRGBChannels(pixels);
  CHECK(ret_split.error().message == "Invalid image dimensions");
}
...
TEST_CASE("Channel expected rgb image") {
  ...
  CHECK(ret_decode.has_value());
  auto pixels = original.pixels();
  ChannelMgr channel_mgr{inputimg.width, inputimg.height, 2};
  auto ret_split = channel_mgr.splitRGBChannels(pixels);
  CHECK(ret_split.error().message == "Expected a three-channel RGB image");
  CHECK(true);
}
